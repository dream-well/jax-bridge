...
contract WjaxEthBridge {
  ...
  uint public minimum_fee_amount = 50; // 50 jaxud
  ...
  IERC20 public jaxud = IERC20(0x643aC3E0cd806B1EC3e2c45f9A5429921422Cd74);
  ...
  function release(
    ...
    jaxud.mint(to, amount - fee_amount);
    if(penalty_amount > 0) {
      if(penalty_amount > fee_amount) {
        jaxud.mint(penalty_wallet, fee_amount);
        penalty_amount -= fee_amount;
      }
      else {
        jaxud.mint(penalty_wallet, penalty_amount);
        jaxud.mint(msg.sender, fee_amount - penalty_amount);
        penalty_amount -= penalty_amount;
      }
    }
    else {
      jaxud.mint(msg.sender, fee_amount);
    }
